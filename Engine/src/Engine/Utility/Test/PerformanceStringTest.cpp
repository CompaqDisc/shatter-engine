...
ETestResult CStringPerformanceTest::Run()
{
	...
	if( !BigTextStream.fail() )
	{
		...
		std::string tmp1 = std::string( TestString );
		const auto& Symbol = TestPool.Find( tmp1 );
		...
		std::string tmp2 = std::string( TestString );
		const std::string& GetString = TestPool.Find( tmp2 );
