...
void CProfiler::PlotPerformance()
{
	...
	if( DrawList && Enabled )
	{
		...
		for( auto& Buffer : PlottableEntries )
		{
			...
			snprintf( TimeEntryName, 1024, "%s (%lli%s)", TimeEntry.Name.String().c_str(), Time, Microsecond ? "us" : "ms" );
...
CTimerScope::~CTimerScope()
{
	...
	else
	{
		...
		FProfileTimeEntry entry = FProfileTimeEntry( ScopeName, int64_t( DeltaTime ), int64_t( StartTimeValue ), Depth );
		CProfiler::Get().AddTimeEntry( entry );
	}
