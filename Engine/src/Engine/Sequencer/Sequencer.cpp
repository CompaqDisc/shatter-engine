		...
		if( DrawTimeline )
		{
			if( ImGui::Begin( "Timeline", &DrawTimeline, ImVec2( 1000.0f, 500.0f ) ) )
			{
				...
				for( auto& Track : Tracks )
				{
					...
					char TrackName[128];
					snprintf( TrackName, 128, "Track %zi", TrackIndex );
					...
					for( auto& Event : Track.Events )
					{
						...
						char EventName[128];
						snprintf( EventName, 128, "%s (Length: %llis)##eb%zi", Event->GetName(), Event->Length / Timebase, GlobalIndex );
						...
						snprintf( ContextName, 128, "ec##%zi", EventIndex );
